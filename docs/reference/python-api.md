...
状态：当前源码候选版本 `0.2.0b1`（尚未公开发布）；领域 Schema 版本
`1.0.0-alpha.1`（`v1alpha`）。Python API 已可运行并有测试覆盖，但尚未承诺 1.0
级别的签名稳定性。普通 Windows 用户应优先使用本机应用，完整工作流的首选自动化入口
仍是 CLI；Python API 适合需要在同一进程内组合、验证或测试底层能力的调用方。
...
| `application`, `session_status` | experimental product façade | 本机四步应用的会话编排、状态重建、恢复与两道人工闸门绑定 |
...
| `app_views`, `app_presenter`, `app_jobs`, `app_server`, `windows_dialogs`, `schemas`, `__about__`, `__main__` | internal product/package layout | 通过本机应用、公开加载器、`__version__` 或 console script 使用，不依赖文件布局 |
