...
- **clean 视图**：`\added` 保留新文字，`\deleted` 去除旧文字，`\replaced{新}{旧}` 只保留新文字；来源规则通过的 `\add` / `\delete` 分别按新增 / 删除处理。它用于生成权威的干净审阅基线。
- **display 视图**：普通纯文本批改精确使用静态蓝字和蓝色单删除线；批改参数含公式、引用、格式或其他结构时按项降级为尽力展示并给出警告。它不是 Word 原生 Track Changes，也不能导入。

检测到批改宏时，程序先保证 `review.docx` 的正文与结构完整。只有普通纯文本批改时，展示
效果不精确仍会阻止发布错误结果；存在已记录的结构化批改时，展示稿若无法通过有界验收可以
不生成，已通过完整性检查的 `review.docx` 仍会发布，并在任务中保留逐项警告。
...
识别到已有 LaTeX 批改宏且展示稿通过验收时，页面还会出现独立的
**已有 LaTeX 批改展示稿（仅供对照）**卡片，并提供“打开”和“另存展示稿”按钮。若结构化
批改只能安全保留干净审阅稿，页面会显示逐项降级警告而不显示这个卡片。展示稿不是上方可发送、
...
| `export/existing-changes-display.docx`（可选） | 对照查看 LaTeX 原稿中已经存在的批改；结构化项目只保证尽力展示 | 不能发回程序作为返回 Word，不能用于自动回填 |

对普通纯文本，展示样式统一使用 `0000FF` 蓝色：新增文字为蓝字；删除文字为蓝色单删除线；
`\replaced{新}{旧}` 显示为旧文字蓝色单删除线后紧接新文字蓝字。本功能不添加高亮。含公式、
引用、格式或其他结构的批改按项产生警告，只提供尽力展示，必要时不生成整份展示稿。
展示稿不是 Word 原生修订，Track Changes 未启用，也不包含原生修订元素。
...
展示稿验收也不是只统计整篇文档中有多少蓝字或删除线。对普通纯文本项，扫描器会生成预期
可见文字和逐字符删除线掩码，DOCX 中必须找到不重叠且文字、蓝色、删除线状态一致的片段；
相同文字重复出现时也必须满足实例数量。删除内容还必须由同段相邻正文唯一证明边界，程序
不会猜测位置。

若批改含公式、引用、格式或其他结构，无法建立上述精确证明的调用会逐项绑定原文件、行号和
源片段并记为警告；精确纯文本子集仍按原门槛核验。展示稿能够通过有界验收时才发布，不能
通过时则省略，`review.docx` 仍是权威交付。

无论是否生成展示稿，程序都会核对干净审阅 Word 的公式、图片、表格、引用/字段和关系。
一旦能证明这些真实论文内容发生静默丢失，导出会硬性停止；不会因为“结构复杂”就降低这条
完整性门槛。
...
普通纯文本参数可包含 Unicode/CJK/emoji、同一段落内的源码换行、嵌套分组、嵌套批改宏和
转义字面量，并接受能够精确投影的简单形式。参数若包含公式、交叉引用、文献引用、格式命令、
环境、脚注、空段落或其他结构，不再仅因此阻断整篇转换：程序会绑定该调用的源位置，生成
逐项警告，并把蓝色展示降为尽力呈现；必要时只交付干净 `review.docx`。

空批改内容（例如 `\added{}`、`\deleted{}` 或 `\replaced{}{}`）会被扫描器如实计入库存，但无法形成可核验的展示证据，因此展示稿导出会以 `E_SCHEMA_INVALID` 安全停止。

危险或越界路径、项目内宏包阴影、`\input@path` 搜索覆盖、无法静态确认的动态宏定义，以及
未闭合或不平衡的动态结构仍会硬性停止；调用处于被跳过的动态条件分支时也不提供检测保证。
这些属于来源身份或路径安全边界，与“结构化批改只降级展示”是两件事。

已有修订宏派生出的文字会进入 clean Word，但不会仅因为来自这些宏就自动获得新的精确
LaTeX SourceMap。审阅者后来若在这类片段中继续修改，变化仍可完整进入账本，但无法证明源
位置时会转人工。自动回填仍只限返回 Word 中经过逐条批准、精确定位的普通正文，并且只写入
新的 LaTeX 副本；程序绝不会用整篇 Word 反向生成结果并覆盖原稿。
...
维护者工作区与公开源码 ZIP 统一双击根目录的 `Start-Latex-Word-Review.cmd`。若
`output\local-windows` 中存在结构完整的本地冻结交付，根启动器会优先转交给冻结版；若没有
或交付不完整，则自动回退到源码准备流程。`LaTeX Word Review（双击启动）.lnk` 可只指向这个
统一入口。本地冻结交付受 Git 忽略，只用于本机验收，不在公开源码 ZIP 中，也不代表 GitHub
已发布二进制。维护者可用 `scripts\deploy-local-windows.ps1` 更新候选；普通用户不需要运行
构建或部署脚本。
...
### 批改宏里有公式或引用，会不会整篇都不能导出

不会仅因为某个批改项含公式、引用、格式或其他结构就阻断整篇。程序仍以干净
`review.docx` 为权威结果，并为每个结构化项给出带源位置的警告；蓝色展示稿只做尽力呈现，
无法安全生成时可以省略。

但如果程序发现危险/越界路径、无法确认的动态宏，或能证明干净 Word 实际丢失了论文中的
图片、公式、表格或引用，仍会停止导出。这是为了防止交付缺内容的 Word，而不是把复杂内容
一概当成错误。
