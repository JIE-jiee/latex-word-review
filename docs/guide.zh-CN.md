...
```text
选择 main.tex
  → 生成 review.docx（干净审阅稿）
      ├─ 通过程序另存精确可编辑副本 → 审阅者在 Word 中使用修订和批注
      │   → 只选择同一轮精确副本的返回 .docx
      └─ 可选：existing-changes-display.docx（已有批改静态展示，只看不回传）
  → 逐条审批（第一道闸门）
  → 核对按文件 diff（第二道闸门）
...
程序会先扫描项目内已绑定的 LaTeX 文件，并生成两种内部视图：

- **clean 视图**：在受支持的静态宏包声明通过有界检查后，`\added` 保留新文字，`\deleted` 去除旧文字，`\replaced{新}{旧}` 只保留新文字；来源规则通过的 `\add` / `\delete` 分别按新增 / 删除处理；用于生成审阅基线。
- **display 视图**：使用静态蓝字和蓝色单删除线保留已有批改的视觉含义；仅当检测到至少一个受支持的批改宏时生成展示稿。它不是 Word 原生 Track Changes，也不能导入。

检测到批改宏时，`review.docx` 与 `existing-changes-display.docx` 会在同一个暂存目录内生成和验收；任一失败都不会发布半套 `export/`。
...
识别到已有 LaTeX 批改宏时，页面还会出现独立的 **已有 LaTeX 批改展示稿（仅供对照）**
卡片，并提供“打开”和“另存展示稿”按钮。这个卡片对应的是静态展示稿，不是上方可发送、
可编辑并回传的审阅副本；Windows 保存窗口也会明确标注“仅供对照”。
...
export\
├── review.docx
└── existing-changes-display.docx（可选，只供对照）
```
...
| 文件 | 可以做什么 | 不可以做什么 |
|---|---|---|
| `export/review.docx` | 作为程序内部密封的 clean 基线；通过程序“另存可编辑 Word”得到精确副本 | 不要直接覆盖密封基线 |
| 审阅 Word 的精确另存副本 | 发给审阅者，并把同一轮返回稿导入当前任务 | 不要跨任务或跨轮次混用 |
| `export/existing-changes-display.docx` | 对照查看 LaTeX 原稿中已经存在的批改 | 不能发回程序作为返回 Word，不能用于自动回填 |

展示样式统一使用 `0000FF` 蓝色：新增文字为蓝字；删除文字为蓝色单删除线；`\replaced{新}{旧}` 显示为旧文字蓝色单删除线后紧接新文字蓝字。本功能不添加高亮。展示稿不是 Word 原生修订，Track Changes 未启用，也不包含原生修订元素。

程序会把展示稿角色 `latex_changes_display_docx`、展示配置和当前任务 ID 写入标准 Word
文档变量，并把最终文件摘要密封进任务状态。导入时先拒绝与展示稿密封摘要完全相同的文件，
再拒绝仍带嵌入展示角色标记的另存或重新打包副本。判断不依赖蓝色本身，因此审阅者在正常
审阅稿中产生的蓝色修订不会仅因颜色被拒绝。不要删除或修改角色标记来尝试绕过检查；即使
第三方工具丢失标记，文件仍必须通过 clean 基线、书签和修订语义核验。

展示稿验收也不是只统计整篇文档中有多少蓝字或删除线。扫描器会从每个最上层批改宏树生成
预期可见文字和逐字符删除线掩码，DOCX 中必须为每项预期找到不重叠且文字、蓝色、删除线状态
一致的片段；相同文字重复出现时也必须满足实例数量。对删除内容，程序还必须从同一段落的
相邻正文唯一证明删除线边界；上下文缺失或出现多个候选位置时会拒绝生成。程序还会比较
clean Word 与展示 Word 新增的蓝字、删除线和高亮数量，并核对公式、图片、表格、字段和关系等
非批改结构。两者还必须保持完整 OPC 部件名集合，以及 `[Content_Types].xml`、样式、编号、
字体、主题和常规设置等受保护部件的语义一致；只排除明确允许的保存标识、经解析的编号定义
标识差异、Track Changes 角色差异和完整精确的展示角色标记。论文别处原有的相同蓝字不能
冒充遗漏的批改；任一门槛失败都会阻止两份 Word 发布。

同段上下文只用于拒绝错误的删除位置，不会建立可供自动回填使用的精确 SourceMap，也不把
某个 LaTeX 调用密码学绑定到 Word 坐标。

### 识别与安全边界

规范命令 `\added`、`\deleted`、`\replaced` 只有在已绑定项目依赖中静态可见标准
`changes` 宏包声明、已绑定项目树未发现同名 `changes.sty`，并且未检测到 `\input@path`
搜索覆盖时才接受。裸规范命令调用以及对规范命令的直接或动态定义/重定义都会以
`E_SCHEMA_INVALID` 在转换前停止。程序不会按文件加载顺序猜测哪一份定义生效。短别名维持
独立的保守静态来源规则：

- `\add` 只接受唯一的静态 `trackchanges` 宏包声明，或唯一的、可静态确认直接转发到
  `\added` 的本地包装；使用宏包声明时，已绑定项目树中的同名 `trackchanges.sty` 和检测到的
  `\input@path` 搜索覆盖都会阻断；
- `\delete` 只接受唯一的、可静态证明直接转发到 `\deleted` 的本地包装，并且目标仍须通过
  上述静态 `changes` 声明门；
- 裸 `\add` / `\delete`、冲突定义、多重来源、签名不兼容或动态条件中的定义都以
  `E_SCHEMA_INVALID` 在转换前停止。

这里的证明范围只是“静态可见的标准宏包声明 + 已绑定项目树中没有发现同名 `.sty` +
没有检测到 `\input@path` 搜索覆盖”。程序不会为此执行 TeX 或 `kpsewhich`，不会解析系统或
用户 TEXMF 树及 `TEXINPUTS`，也不绑定 TeX 最终加载的宏包路径、版本或哈希。因此它不是对
电脑上实际宏包文件身份的证明。

别名来源会在已绑定的 UTF-8 `.tex`、`.sty`、`.cls` 中核查。主文件的批改调用只扫描
`document` 正文，其他已绑定的 UTF-8 `.tex` 文件扫描全文。注释、`\verb`、`verbatim`、
`verbatim*`、`Verbatim`、`minted`、`lstlisting`、定义命令的正文，以及动态条件的全部分支
不会被当作调用；目标规范命令与短别名的定义仍会接受来源和冲突审计。普通包装宏和 `\csname`
不会展开，TeX 条件也不会求值。动态区域中的调用不在检测保证内。

安全参数可包含普通 Unicode/CJK/emoji、同一段落内的普通源码换行、普通嵌套分组、嵌套批改宏、转义字面量和受支持的简单内联格式；空行形成的段落分隔不允许。直接识别到的参数若包含公式、交叉引用、文献引用、图片、环境、脚注、空段落或其他结构命令，程序会在转换后端运行前以 `E_SCHEMA_INVALID` 停止。

空批改内容（例如 `\added{}`、`\deleted{}` 或 `\replaced{}{}`）会被扫描器如实计入库存，但无法形成可核验的展示证据，因此展示稿导出会以 `E_SCHEMA_INVALID` 安全停止。

已有修订宏派生出的文字会进入 clean Word，但不会仅因为来自这些宏就自动获得新的精确 LaTeX SourceMap。审阅者后来若在这类片段中继续修改，变化仍可完整进入账本，但无法证明源位置时会转人工，不能承诺自动回填。
...
| 选择了展示稿或保留其角色标记的副本 | 命中密封摘要或嵌入展示角色 | 只选择同一任务中由 `review.docx` 精确另存并经审阅后返回的文件 |
...
| 结果 | 说明 |
|---|---|
| 已有批改展示 Word（可选） | `existing-changes-display.docx`，只供审阅前对照，不能作为返回稿 |
...
### 可以把 `existing-changes-display.docx` 当成返回 Word 吗

不可以。只有 `review.docx` 的精确另存可编辑副本属于返回审阅角色。展示稿的最终摘要会被
密封，文件内也带有标准 Word 文档变量形式的角色标记；完全相同的展示稿和仍保留标记的另存
副本都会在归档、解析和审批之前被拒绝。蓝色格式本身不作为判断依据。
