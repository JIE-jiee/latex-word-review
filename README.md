...
| 查看 LaTeX 中已有的批改标记 | 在通过受限静态检查后识别 `\added`、`\deleted`、`\replaced`，并保守兼容 `\add`、`\delete`；另外生成统一 `0000FF` 蓝色的静态展示 Word |
...
程序只做有界静态检查：源码中必须有标准 `changes` 宏包声明，已绑定项目树中不能发现同名
`changes.sty`，也不能检测到 `\input@path` 搜索路径覆盖，之后才识别
`\added{新文字}`、`\deleted{旧文字}`、`\replaced{新文字}{旧文字}`。裸调用以及对这些命令的
直接或动态定义/重定义都会在转换前停止。`\add`、`\delete` 仍按更保守的静态声明或直接包装
规则处理；`trackchanges.sty` 阴影、来源冲突或无法确认时不猜测。

| 文件 | 作用 |
|---|---|
| `review.docx` 及程序生成的精确可编辑副本 | 干净的审阅稿；只有这个审阅角色的另存副本可以发出并作为返回 Word 导入 |
| `existing-changes-display.docx` | 只用于查看 LaTeX 中已经存在的批改：新增文字为 `0000FF` 蓝字，删除文字为 `0000FF` 蓝色单删除线；替换显示为旧文字蓝色单删除线后紧接新文字蓝字。本功能不添加高亮 |

展示稿不是 Word 原生“修订/Track Changes”，不能作为返回 Word 导入，也不参与自动回填。
生成前后，程序会核对源文字、蓝字与删除线效果，以及公式、图片、表格和字段等非批改结构；
任一检查失败，两份 Word 都不会写入正式任务结果。颜色本身不用于判断文件角色，正常审阅稿
里的蓝字不会因此被拒绝。对删除内容，程序还必须从同一段落的相邻正文唯一证明删除线所在
位置；上下文缺失或有多个可能位置时会拒绝生成，而不是把删除线放到猜测的位置。

这里的“静态检查”只证明项目中看到了受支持的标准宏包声明，并且没有发现上述项目内阴影或
搜索路径覆盖；它不会运行 TeX，也不证明电脑的系统/用户 TEXMF 或 `TEXINPUTS` 最终加载了
哪个包文件、版本或哈希。空批改内容（如 `\added{}`、`\deleted{}`、`\replaced{}{}`）也无法
形成可核验的展示证据，会以 `E_SCHEMA_INVALID` 安全停止。

两份 Word 还会对照完整 OPC 部件集合，以及样式、编号、字体、主题、内容类型和常规设置等
受保护部件；除已明确允许的保存标识和展示稿角色标记外，相关内容发生漂移就不会发布。

这项功能不会建立“某个 LaTeX 调用到某个 Word 坐标”的新 SourceMap。审阅者之后再修改这类
文字时，如果程序不能证明 LaTeX 位置，修改会保留在记录中并转为人工处理，而不会冒险自动
回填。扫描范围、别名来源证明、结构内容限制和完整验收门槛见
...
- **批改展示 Word 只供静态对照。** 它不是 Word 原生 Track Changes，绝不能作为返回稿；文件摘要与嵌入角色标记都会参与拒绝。识别到批改宏时，两份 Word 必须在同一次暂存导出中通过源派生文字、clean/display 样式增量、删除位置的同段唯一上下文证明与非批改结构验收后才会发布。
