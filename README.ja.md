...
- LaTeX に既に書かれている変更マークを確認できます。制限付きの静的宣言チェックを通過した `\added`、`\deleted`、`\replaced` を認識し、`\add`、`\delete` はより保守的に扱って、`0000FF` の青色で統一した表示専用 Word を別途生成します。
...
プロジェクト内で標準 `changes` パッケージの宣言を静的に確認でき、探索対象のプロジェクト
ツリーに同名の `changes.sty` がなく、検出可能な `\input@path` 上書きもない場合だけ、
`\added{新しい文字}`、`\deleted{古い文字}`、`\replaced{新しい文字}{古い文字}` を認識します。
根拠のない標準名の呼び出しや、標準名の直接または動的な定義・再定義は、変換前に安全側で
停止します。`\add` と `\delete` は、さらに保守的な静的宣言または直接ラッパー規則で扱い、
`trackchanges.sty` による隠蔽、由来の競合、確認できない由来があれば推測しません。

| ファイル | 用途 |
|---|---|
| `review.docx` と、その正確な編集用保存コピー | クリーンなレビュー文書です。このレビュー用途の保存コピーだけを査読者へ渡し、返却 Word として読み込めます |
| `existing-changes-display.docx` | LaTeX に既にある変更の表示専用 Word です。追加は `0000FF` の青字、削除は `0000FF` の青色一重取り消し線、置換は古い青色一重取り消し線の文字の直後に新しい青字を表示します。この機能は蛍光ペンを追加しません |

表示専用 Word は Word の変更履歴ではなく、返却 Word として読み込めず、自動書き戻しにも
使いません。二つの Word を正式なタスク結果にする前に、ソース由来の文字と青字・取り消し線の
効果、および数式・画像・表・フィールドなど変更以外の構造を検証します。どれか一つでも失敗
すると、両方の結果を確定しません。青色だけで表示稿を判定しないため、通常の査読稿に青色の
変更履歴があっても、それだけを理由に拒否しません。削除文字については、同じ段落の前後の
本文から取り消し線の位置を一意に証明できることも必要です。文脈が足りない、または候補が
複数ある場合は、位置を推測せず生成を拒否します。

この確認は、制限付きの構文検査です。TeX は実行せず、システムまたはユーザーの TEXMF ツリーや
`TEXINPUTS` を解決しないため、TeX が最終的に読み込むパッケージのパス、版、ハッシュまでは
証明しません。`\added{}`、`\deleted{}`、`\replaced{}{}` のような空の変更は表示証拠を作れず、
`E_SCHEMA_INVALID` で安全側に停止します。

二つの Word は、OPC 部品名の集合に加えて、スタイル、番号、フォント、テーマ、コンテンツ型、
一般設定などの保護対象部品も一致する必要があります。明示的に許可した保存識別子と正確な
表示用途マーカーを除き、これらの部品が変わっていれば公開しません。

この機能は、一つの LaTeX 呼び出しを Word 内の一つの座標へ結び付ける新しい SourceMap を
作りません。後から編集した文字の LaTeX 側位置を証明できない場合は、変更を記録して手動処理へ
回します。エイリアスの由来、スキャン範囲、構造内容の制限、ファイル役割、検証条件の詳細は
...
- 表示専用 Word は静的な青字・一重取り消し線による参照専用ファイルであり、Word のネイティブ変更履歴ではなく、返却稿として読み込めません。封印ダイジェストと埋め込み役割マーカーの両方を読み込み拒否に使います。変更マクロを検出した場合、2 つの Word が同じステージング書き出しでソース由来文字列、clean/display 書式差分、削除位置を証明する同一段落内の一意な文脈、変更以外の構造検証に合格してから結果を公開します。
